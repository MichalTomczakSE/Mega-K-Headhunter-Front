	...
	return (
			<Container >
					...
					<div className='flex flex-col h-3/4 w-9/12'>
						<div className='py-2 h-[85px] px-2 border-b-primary-background border-b-[3px] flex text-center flex-row justify-between items-center w-full  h-3/4 bg-secondary-background '>
							 <SearchStudent setSearch={(search)=> setSearch(search)} />
							 <Filter />
						</div>
						<Pagination total={studentsCount ? studentsCount : 0} limit={perPage} setLimit={perPage => setPerPage(perPage)} page={currentPage} setPage={currentPage => setCurrentPage(currentPage)} />
				</div>
			</Container >
	)
};
